...
for link in URLs:
    ...
    for proxyVarient in varients:
        ...
        ids_classes = [] #list of both classes and ids
            ...
            for t in tags:
                ...
                try:
                    ...
                    if cl != None and (not cl in classes) and (type(cl)==type('str') or (type(cl)==type(['list']) and len(cl)==1)) :
                        ...
                        ids_classes.append([tag,'class',cl])
                except:
                    pass
        ...
        for idclass in ids_classes:
            try:
                # find element in the code
                elem = driver.find_element_by_xpath("//{}[@{}='{}']".format(idclass[0],idclass[1],idclass[2]))
                ...
                if not os.path.exists("ss/" + folderName):
                    ...
                    os.mkdir("ss/" + folderName + "/1")
                    os.mkdir("ss/" + folderName + "/2")
                ssPath = "ss/" + folderName + "/" + str(varients.index(proxyVarient)) + "/screenshot{}-{}-{}.png".format(idclass[0],idclass[1],idclass[2])
                driver.save_screenshot(ssPath)
                print ("Capturing Screenshot of {} = {}".format(idclass[1],idclass[2]))
                time.sleep(.5)
            except:
                print ("Error parsing of {} = {}".format(idclass[1],idclass[2]))
